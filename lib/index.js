...
var composeTransforms = (opts) => {
  ...
  if (hasScale) {
    ...
    const [baseFx, baseFy] = translatePair ? translatePair.from.split(" ").map(Number) : [Number(from.x ?? 0), Number(from.y ?? 0)];
    const [baseTx, baseTy] = translatePair ? translatePair.to.split(" ").map(Number) : [Number(to.x ?? 0), Number(to.y ?? 0)];
    translatePair = {
      type: "translate",
      from: translateStr(baseFx + originX * (1 - fromSx) - fromSx * originY * fromTanSkewX, baseFy + originY * (1 - fromSy) - fromSy * originX * fromTanSkewY),
      to: translateStr(baseTx + originX * (1 - toSx) - toSx * originY * toTanSkewX, baseTy + originY * (1 - toSy) - toSy * originX * toTanSkewY)
    };
  }
  const pairs = [
    translatePair,
...
class SMILTween extends Animation {
  ...
  _build = () => {
    ...
    for (let i = 0;i < this._targets.length; i++) {
      ...
      const elements = [
        ...
        ...this._buildDirect(direct, fromRouted?.direct, beginDelay, groupDuration, this._yoyo ? this._originalValues.get(target) ?? undefined : undefined)
      ];
      if (hasStaggerRepeat) {
  ...
  _buildObjectArrayKeyframes = (steps) => {
    ...
    this._tDur = this._repeat === -1 ? Infinity : totalStepDur * (this._repeat + 1) + this._rDelay * this._repeat;
    this._initialized = true;
  };
    ...
    for (let i = 0;i < this._targets.length; i++) {
      ...
      if (hasTransforms) {
        ...
        if (hasTranslate) {
          ...
          const values = x.map((xi, i2) => `${xi} ${y[i2]}`).join("; ");
        ...
        if (hasScale) {
          ...
          const values = sx.map((s, i2) => `${s} ${sy[i2]}`).join("; ");
  ...
  _buildPercentageKeyframes = (kf) => {
    ...
    if (stops.length === 0)
      return;
    if (stops[0].pct > 0)
      stops.unshift({ pct: 0, vars: {} });
    if (stops[stops.length - 1].pct < 1)
      stops.push({ pct: 1, vars: {} });
    if (stops.length < 2)
      return;
    ...
    for (let i = 0;i < this._targets.length; i++) {
      ...
      if (hasTransforms) {
        originals["transform"] = target.getAttribute("transform");
        const { cx: originX, cy: originY } = hasScale ? resolveRotationOrigin(target, this._vars.transformOrigin) : { cx: 0, cy: 0 };
        if (hasTranslate || hasScale) {
          ...
          const values = routedStops.map(({ transforms }) => {
            ...
            if (sy !== undefined)
              lastSy = Number(sy);
            if ("skewX" in transforms)
              lastSkewX = Number(transforms.skewX ?? 0);
            if ("skewY" in transforms)
              lastSkewY = Number(transforms.skewY ?? 0);
    ...
    if (this._yoyo) {
      ...
      const repeatCount = totalPlays === Infinity ? "indefinite" : totalPlays % 2 === 0 ? String(totalPlays / 2) : "1";
