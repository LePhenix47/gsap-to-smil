...
class TriggerResolver {
  ...
  static resolveTarget = (target, firstTarget) => {
    ...
    if (bySelector)
      return bySelector;
    console.warn(`[gsap-to-smil] Trigger target "${target}" not found in DOM. Falling back to first tween target.`);
    return firstTarget;
  };
...
class SMILTimeline extends Animation {
  ...
  to = (targetParam, toVars, position) => {
    const mergedVars = { ...this._defaults, ...toVars, delay: 0 };
  ...
  getChildren = () => this._children.map((entry) => entry.tween);
  clear = () => {
