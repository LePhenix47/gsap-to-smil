...
class PropertyRouter {
  ...
  static PLUGIN_KEYS = new Set(["drawSVG", "motionPath", "morphSVG"]);
...
class Easing {
  ...
  static uniformKeyTimes(keyTimesAmount) {
  ...
  static keySplines(timingAnimation, keyTimesAmount) {
    if (!keyTimesAmount) {
      throw new Error("[gsap-to-smil] keyTimesAmount must be a positive integer");
    }
    const bezier = Easing.resolveEase(timingAnimation);
    if (!bezier) {
      throw new Error(`[gsap-to-smil] "${timingAnimation}" resolves to linear or null — use calcMode="linear" instead of keySplines`);
    }
  ...
  static timingFunctionString(valuesAmount, timingAnimation) {
...
class SMILBuilder {
  ...
  static applyEasing(el, ease, intervalCount) {
  ...
  static animate(opts) {
    const el = document.createElementNS(SMILBuilder.SVG_NS, "animate");
  ...
  static animateTransform(opts) {
    const el = document.createElementNS(SMILBuilder.SVG_NS, "animateTransform");
  ...
  static set(attributeName, to, delay) {
    const el = document.createElementNS(SMILBuilder.SVG_NS, "set");
    el.setAttribute("attributeName", attributeName);
    el.setAttribute("to", to);
    if (delay)
      el.setAttribute("begin", `${delay}s`);
    return el;
  }
  static injectInto(target, ...animations) {
...
class TransformComposer {
  ...
  static resolvePercent(value, dim, el) {
  ...
  static translateStr(x, y) {
    return `${x} ${y}`;
  }
  static rotateStr(angle, cx, cy) {
    return `${angle} ${cx} ${cy}`;
  }
  static scaleStr(sx, sy) {
    return `${sx} ${sy}`;
  }
  static resolveTranslate(from, to, target) {
    if (!(("x" in to) || ("y" in to) || ("xPercent" in to) || ("yPercent" in to)))
      return null;
    const toX = to.xPercent !== undefined ? TransformComposer.resolvePercent(to.xPercent, "width", target) : to.x ?? 0;
    const toY = to.yPercent !== undefined ? TransformComposer.resolvePercent(to.yPercent, "height", target) : to.y ?? 0;
    const fromX = from.xPercent !== undefined ? TransformComposer.resolvePercent(from.xPercent, "width", target) : from.x ?? 0;
    const fromY = from.yPercent !== undefined ? TransformComposer.resolvePercent(from.yPercent, "height", target) : from.y ?? 0;
    return {
      type: "translate",
  ...
  static resolveRotate(from, to, cx, cy) {
    if (!("rotation" in to))
      return null;
    return {
      type: "rotate",
  ...
  static resolveScale(from, to) {
    ...
    return {
      type: "scale",
      from: TransformComposer.scaleStr(from.scale ?? from.scaleX ?? 1, from.scale ?? from.scaleY ?? 1),
  ...
  static buildPivotScaffold(el, cx, cy) {
  ...
  static compose(opts) {
    ...
    const make = (pair) => SMILBuilder.animateTransform({
      type: pair.type,
      from: pair.from,
      to: pair.to,
      additive: "sum",
...
class DrawSVGPlugin {
  ...
  static buildAnimation(target, toVal, fromVal, opts) {
    target.setAttribute("pathLength", "1");
    const toState = DrawSVGPlugin.parseValue(toVal);
    const fromState = (() => {
      if (fromVal !== undefined)
        return DrawSVGPlugin.parseValue(fromVal);
...
class SMILTween extends Animation {
  ...
  _build = () => {
    ...
    const staggerDelays = this._vars.stagger ? StaggerResolver.resolveDelays(this._targets.length, this._vars.stagger) : null;
    ...
    for (let i = 0;i < this._targets.length; i++) {
      ...
      const drawAnims = (() => {
        ...
        return DrawSVGPlugin.buildAnimation(target, this._isFrom ? true : plugins.drawSVG, this._isFrom ? plugins.drawSVG : fromRouted?.plugins.drawSVG, timingOpts);
      })();
      const elements = [
        ...outerAnims,
        ...innerAnims,
  ...
  _buildDirect = (direct, fromDirect, delay, dur, originals) => {
    ...
    for (const [attr, value] of Object.entries(direct)) {
      ...
      if (this._isFrom) {
        ...
        elements.push(SMILBuilder.animate({
          ...shared,
