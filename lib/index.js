...
class SMILTimeline extends Animation {
  ...
  to = (targetParam, toVars, position) => {
    ...
    const accumulated = firstTarget !== undefined ? this.accumulatedTransforms.get(firstTarget) ?? {} : {};
    const deltaVars = this.computeDeltaTransformVars(mergedVars, accumulated);
    const adjustedOrigin = firstTarget !== undefined ? this.computeAdjustedTransformOrigin(firstTarget, mergedVars, accumulated) : undefined;
    const tweenVars = {
      ...deltaVars,
      trigger: undefined,
  ...
  computeDeltaTransformVars = (toVars, accumulated) => {
    const additiveKeys = [
      "x",
      "y",
      "xPercent",
    ...
    const hasAdditiveKey = additiveKeys.some((key) => !this.isAbsent(toVars[key]));
    if (!hasAdditiveKey)
      return toVars;
    const deltaVars = { ...toVars };
    for (const key of additiveKeys) {
      const rawValue = toVars[key];
      if (this.isAbsent(rawValue))
        continue;
      const accumulatedValue = Number(accumulated[key] ?? 0);
      deltaVars[key] = Number(rawValue) - accumulatedValue;
    }
    return deltaVars;
  };
  computeAdjustedTransformOrigin = (target, toVars, accumulated) => {
    const needsPivot = !this.isAbsent(toVars.scale) || !this.isAbsent(toVars.scaleX) || !this.isAbsent(toVars.scaleY) || !this.isAbsent(toVars.skewX) || !this.isAbsent(toVars.skewY);
    if (!needsPivot || !this.isAbsent(toVars.transformOrigin))
      return;
    ...
    const accumulatedX = Number(accumulated.x ?? 0);
    const accumulatedY = Number(accumulated.y ?? 0);
    return `${bboxCenterX + accumulatedX} ${bboxCenterY + accumulatedY}`;
  };
  updateAccumulatedTransforms = (targets, toVars) => {
    const trackedKeys = [
      "x",
      "y",
      "xPercent",
    ...
    for (const target of targets) {
      const existing = this.accumulatedTransforms.get(target) ?? {};
