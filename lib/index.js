...
class PropertyRouter {
  ...
  static route(vars) {
    ...
    for (const [key, value] of Object.entries(vars)) {
      if (key === "attr") {
        console.warn("[gsap-to-smil] The `attr` key has not been implemented yet and will be ignored.");
        continue;
      }
...
class SMILTween extends Animation {
  ...
  buildElements = (toVars, buckets) => {
    ...
    for (const target of this.targetElements) {
      ...
      this.buildDirectProperties(target, buckets, fromBuckets, totalDelay, { ease });
      if (hasTransforms) {
        this.buildTransformElements(target, toTransforms, fromTransforms ?? undefined, totalDelay, transformOrigin, { ease }, this.isFromTween);
      }
    }
  };
  buildYoyoElements = (toVars, buckets, fromBuckets) => {
    ...
    for (const target of this.targetElements) {
      for (const [attributeName, toValue] of Object.entries(buckets.direct)) {
        ...
        const { values, dur, repeatCount } = SMILTween.computeYoyoEncoding(fromValue, String(toValue), totalPlays, durationSeconds, this.repeatCount);
  ...
  buildDirectProperties = (target, buckets, fromBuckets, delay, timing) => {
    const { ease } = timing;
  ...
  buildTransformElements = (target, toTransforms, fromTransforms, delay, transformOrigin, timing, invertDirection = false) => {
    const { ease } = timing;
    const result = TransformComposer.compose({
      toTransforms,
      fromTransforms,
