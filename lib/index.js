...
class SMILTween extends Animation {
  ...
  _build = () => {
    ...
    for (let i = 0;i < this._targets.length; i++) {
      ...
      const { outerAnims, innerAnims, scaffold } = this._buildTransforms(target, transforms, fromRouted?.transforms, beginDelay, groupDuration);
  ...
  _buildTransforms = (target, transforms, fromTransforms, delay, dur) => {
    ...
    const composeArgs = this._isFrom ? { target, fromTransforms: transforms, toTransforms: this._identityFor(transforms), transformOrigin: this._vars.transformOrigin, ...timing } : fromTransforms ? { target, fromTransforms, toTransforms: transforms, transformOrigin: this._vars.transformOrigin, ...timing } : { target, toTransforms: transforms, transformOrigin: this._vars.transformOrigin, ...timing };
