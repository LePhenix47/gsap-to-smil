...
describe("builders", () => {
  ...
  describe("buildSet", () => {
    ...
    it("HAPPY PATH: creates a <set> element", () => {
      const result = SMILBuilder.set("opacity", "0");
      const expected = "set";
    ...
    it("HAPPY PATH: sets attributeName and to", () => {
      const result = SMILBuilder.set("fill", "red");
    ...
    it("HAPPY PATH: sets begin when a delay is provided", () => {
      ...
      const result = SMILBuilder.set("opacity", "0", delay);
      const expected = `${delay}s`;
    ...
    it("EDGE CASE: delay=0 produces no begin attribute", () => {
      const result = SMILBuilder.set("opacity", "0", 0);
  ...
  describe("injectInto", () => {
    ...
    it("HAPPY PATH: appends a single animation element as a child of the target", () => {
      const target = document.createElementNS(SMILBuilder.SVG_NS, "circle");
      const anim = SMILBuilder.animate({ attributeName: "opacity", to: "0", dur: 1 });

      SMILBuilder.injectInto(target, anim);
    ...
    it("HAPPY PATH: appends multiple animations in order", () => {
      const target = document.createElementNS(SMILBuilder.SVG_NS, "rect");
      const anim1 = SMILBuilder.animate({ attributeName: "opacity", to: "0", dur: 1 });
      const anim2 = SMILBuilder.animateTransform({
        type: "translate",
        from: "0 0",
        to: "100 0",
        dur: 1,
      ...
      SMILBuilder.injectInto(target, anim1, anim2);
