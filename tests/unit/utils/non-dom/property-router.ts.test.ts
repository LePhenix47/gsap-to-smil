...
describe("property-router", () => {
  describe("routeProperties", () => {
    ...
    it.todo(
      "HAPPY PATH: yoyo: true routes to special bucket and SMILTween produces ping-pong animation",
    );
    it.todo(
      "HAPPY PATH: yoyo: false (default) produces a normal forward-only animation",
    );
    it.todo(
      "EDGE CASE: yoyoEase overrides the return-trip ease when yoyo is active",
    );
  });
});
