...
describe("stagger-resolver", () => {
  describe("resolveStaggerDelays", () => {
    ...
    it("HAPPY PATH: simple number stagger produces evenly spaced delays from index 0", () => {
      const result = resolveStaggerDelays(3, 0.1);
      const value = [0, 0.1, 0.2];

      expect(result).toEqual(value);
    });

    it("HAPPY PATH: { each } form behaves identically to a plain number stagger", () => {
      const result = resolveStaggerDelays(3, { each: 0.1 });
      const value = [0, 0.1, 0.2];
    ...
    it("HAPPY PATH: { amount } form spreads total delay evenly across all targets", () => {
      ...
      const result = resolveStaggerDelays(3, { amount: 1 });
      const value = [0, 0.5, 1];
    ...
    it("HAPPY PATH: from:'start' is the default — delays increase left to right", () => {
      ...
      const result = resolveStaggerDelays(4, { each: 0.5, from: "start" });
      const value = [0, 0.5, 1, 1.5];

      expect(result).toEqual(value);
    });

    it("HAPPY PATH: from:'end' reverses the order — last element starts first", () => {
      const result = resolveStaggerDelays(3, { each: 0.1, from: "end" });
      const value = [0.2, 0.1, 0];
    ...
    it("HAPPY PATH: from:'center' fans out from the middle element", () => {
      ...
      const result = resolveStaggerDelays(5, { each: 0.1, from: "center" });
      const value = [0.2, 0.1, 0, 0.1, 0.2];
    ...
    it("HAPPY PATH: from:'center' with even count uses the midpoint between the two center elements", () => {
      ...
      const result = resolveStaggerDelays(4, { each: 0.5, from: "center" });
      const value = [0.75, 0.25, 0.25, 0.75];
    ...
    it("HAPPY PATH: from:'edges' fans in from both ends toward the center", () => {
      ...
      const result = resolveStaggerDelays(4, { each: 0.1, from: "edges" });
      const value = [0, 0.1, 0.1, 0];
    ...
    it("HAPPY PATH: from: numeric index uses that element as the origin", () => {
      ...
      const result = resolveStaggerDelays(3, { each: 0.1, from: 1 });
      const value = [0.1, 0, 0.1];
    ...
    it("EDGE CASE: count=0 returns an empty array regardless of stagger config", () => {
      const resultA = resolveStaggerDelays(0, 0.1);
      const resultB = resolveStaggerDelays(0, { each: 0.5 });
      const value: number[] = [];

      expect(resultA).toEqual(value);
      expect(resultB).toEqual(value);
    });

    it("EDGE CASE: count=1 always returns [0] — single element has no stagger offset", () => {
      const resultA = resolveStaggerDelays(1, 0.5);
      const resultB = resolveStaggerDelays(1, { each: 0.5 });
      const value = [0];
    ...
    it("EDGE CASE: { amount } with count=1 returns [0] — division by zero guard", () => {
      ...
      const result = resolveStaggerDelays(1, { amount: 1 });
      const value = [0];
    ...
    it("EDGE CASE: each:0 produces all-zero delays regardless of from", () => {
      const result = resolveStaggerDelays(4, { each: 0, from: "end" });
      const value = [0, 0, 0, 0];
    ...
    it("EDGE CASE: StaggerObject with neither each nor amount defaults to a step of 0", () => {
      const result = resolveStaggerDelays(3, {});
      const value = [0, 0, 0];
