...
**Where the checklist came from:** the items match the **auto transcript** published with that YouTube video (search / page text tools can retrieve it). So the order and substance follow the video; only obvious transcript bugs were adjusted (e.g. “cloud.md” → `CLAUDE.md`, and the duplicate “#28” in the video split into items 28 and 29 below).
